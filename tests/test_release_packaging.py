...
class ReleasePackagingTests(unittest.TestCase):
    def test_firmware_version_comes_from_product_config(self):
        self.assertEqual("1.2.0", PACKAGE_INSTALLER.firmware_version())
    ...
    def test_package_copies_page_and_device_catalog_assets(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for filename in (
                ...
                "manifest.json",
                "ota-manifest.json",
                "firmware/transitink-zectrix-note4-ota-v1.2.0.bin",
            ...
            bundle = output / "transitink-zectrix-note4-v1.2.0.zip"
            self.assertTrue(bundle.is_file())
            with zipfile.ZipFile(bundle) as archive:
                names = set(archive.namelist())
            self.assertIn("transitink-zectrix-note4-v1.2.0.bin", names)
            self.assertIn("transitink-zectrix-note4-ota-v1.2.0.bin", names)
